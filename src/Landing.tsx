...
// ─── Dados estáticos ────────────────────────────────────────────────────────
const services = [
  { title: "Sites Institucionais", desc: "Sites rápidos, responsivos e otimizados para o Google. Sua marca com cara de gente grande." },
  { title: "Softwares Locais", desc: "Sistemas sob medida para o seu comércio: caixa, estoque, agendamentos e muito mais." },
  { title: "Landing Pages", desc: "Páginas de venda que convertem. Copy persuasiva e design que prende o olhar." },
  { title: "UI / UX Design", desc: "Interfaces bonitas e fáceis de usar. Seu produto com experiência de aplicativo top." },
  { title: "Posts para Instagram", desc: "Feed coeso, carrosséis que prendem atenção e stories estratégicos." },
  { title: "Identidade Visual", desc: "Logos, paletas e tipografia que fazem sua marca ser inesquecível." },
];

const steps = [
  { n: "1", t: "Conversa", d: "A gente entende seu negócio, seus clientes e objetivos numa call descontraída." },
  { n: "2", t: "Proposta", d: "Você recebe um plano claro com prazos, valores e tudo que vai ser entregue." },
  { n: "3", t: "Criação", d: "Mão na massa: design, código e revisões com você acompanhando cada passo." },
  { n: "4", t: "Entrega", d: "Tudo no ar, com suporte e orientação para você continuar crescendo." },
];
...
const faqs = [
  { q: "Quanto tempo leva para ficar pronto?", a: "Landing pages saem em 5–10 dias. Sites completos entre 2 e 4 semanas. Softwares variam conforme o escopo — combinamos tudo na proposta." },
  { q: "Posso pedir mudanças depois?", a: "Sim! Sempre que precisar atualizar textos, imagens ou seções, fazemos as alterações para você." },
  { q: "Vocês fazem só projetos grandes?", a: "Não. Atendemos desde o autônomo até comércios locais e startups. Tem plano para cada tamanho de sonho." },
  { q: "E se eu não souber o que quero?", a: "Sem problema. Na conversa inicial te ajudamos a desenhar a ideia. Trazemos referências e direções para você escolher." },
];

const marqueeItems = ["Social Media", "Softwares Locais", "Landing Pages", "UI/UX Design", "Posts Instagram", "Identidade Visual"];
  ...
  return (
    <div className="grain min-h-screen">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-background/70 border-b border-ink/10">
        ...
        >
          <a href="#" className="flex items-center gap-2" aria-label="Koala Studio — Página inicial">
            <img
              src={logoNav}
              alt=""
              aria-hidden="true"
          ...
          <div className="hidden md:flex items-center gap-7 text-sm font-medium" role="list">
            <a href="#servicos" className="hover:text-eucalyptus transition" role="listitem">Serviços</a>
            <a href="#processo" className="hover:text-eucalyptus transition" role="listitem">Processo</a>
            <a href="#planos" className="hover:text-eucalyptus transition" role="listitem">Planos</a>
          </div>
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="max-w-6xl mx-auto px-5 py-4 flex flex-col gap-4 text-sm font-medium">
              <a href="#servicos" onClick={() => setMobileMenuOpen(false)} className="hover:text-eucalyptus transition py-1">Serviços</a>
              <a href="#processo" onClick={() => setMobileMenuOpen(false)} className="hover:text-eucalyptus transition py-1">Processo</a>
              <a href="#planos" onClick={() => setMobileMenuOpen(false)} className="hover:text-eucalyptus transition py-1">Planos</a>
            </div>
          </div>
        )}
      </header>
      ...
      <main id="main-content">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="mt-6 text-lg text-muted-foreground max-w-lg">
                A gente cria sites, softwares locais, landing pages, design UI/UX e posts para Instagram.
                Tudo bonito, fácil de mexer e pensado para você vender mais.
              </p>
        ...
        >
          ...
          >
            {[0, 1].map((i) => (
              <div key={i} className="flex gap-12" aria-hidden={i > 0 ? "true" : undefined}>
                {marqueeItems.map((item) => (
                  <span key={item}>{item} <span aria-hidden="true">·</span> </span>
                ))}
              </div>
            ))}
          </div>
        ...
        >
          <div className="max-w-2xl">
            <p className="text-sm font-semibold text-eucalyptus uppercase tracking-wider">O que entregamos</p>
            <h2 id="services-heading" className="mt-3 text-4xl md:text-5xl font-bold">
        ...
        <Suspense fallback={
          <section className="max-w-6xl mx-auto px-5 py-24" aria-label="Carregando projetos">
            <div className="h-96 rounded-2xl bg-eucalyptus-soft/40 animate-pulse" aria-hidden="true" />
          </section>
        }>
          <ProjectsCarousel />
        </Suspense>
        ...
        >
          <div className="max-w-6xl mx-auto px-5">
            <div className="max-w-2xl">
              <p className="text-sm font-semibold uppercase tracking-wider">Como funciona</p>
              <h2 id="process-heading" className="mt-3 text-4xl md:text-5xl font-bold">
        ...
        >
          <div className="text-center max-w-2xl mx-auto">
            <p className="text-sm font-semibold text-eucalyptus uppercase tracking-wider">Planos</p>
            <h2 id="plans-heading" className="mt-3 text-4xl md:text-5xl font-bold">
          ...
          <div className="mt-14 grid md:grid-cols-3 gap-6">
            {plans.map((p) => (
              ...
              >
                ...
                <ul className="mt-6 space-y-3 text-sm" aria-label={`Recursos do plano ${p.name}`}>
                  {p.features.map((f) => (
                    <li key={f} className="flex gap-2">
                      <span className={p.highlight ? "text-eucalyptus-soft" : "text-eucalyptus"} aria-hidden="true">✓</span>
                      <span>{f}</span>
                    </li>
                  ))}
                </ul>
        ...
        >
          <div className="text-center">
            <p className="text-sm font-semibold text-eucalyptus uppercase tracking-wider">Dúvidas</p>
            <h2 id="faq-heading" className="mt-3 text-4xl md:text-5xl font-bold">
            ...
            {faqs.map((f) => (
              <details key={f.q} className="neo-card p-6 group">
                <summary className="cursor-pointer list-none flex items-center justify-between font-bold text-lg">
                  {f.q}
                  <span className="text-2xl group-open:rotate-45 transition-transform" aria-hidden="true">+</span>
                </summary>
        ...
        >
          <div className="neo-card bg-ink text-cream p-10 md:p-16 text-center relative overflow-hidden">
            <img
              ...
              loading="lazy"
            />
            <h2 id="cta-heading" className="font-display text-4xl md:text-6xl font-bold">
            ...
            <div className="mt-8 flex flex-wrap gap-4 justify-center">
              ...
              <a
                href="mailto:ola@koala.studio"
                className="neo-btn bg-cream text-ink px-7 py-3.5 font-semibold inline-flex items-center gap-2"
                aria-label="Enviar e-mail para ola@koala.studio"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5" aria-hidden="true" focusable="false">
      ...
      <footer className="bg-ink text-cream" aria-label="Rodapé">
        <div className="max-w-6xl mx-auto px-5 py-16">
          ...
          <div className="mt-12 pt-6 border-t border-cream/10 flex flex-col md:flex-row items-center justify-between gap-3 text-sm text-cream/80">
            ...
            <p>Feito <span className="text-white" aria-hidden="true">💚</span> em São Carlos, São Paulo, Brasil</p>
          </div>
        </div>
      </footer>
    </div>
